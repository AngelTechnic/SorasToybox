    ...
    public class EntrenchingTool
    {
        public static void Add()
        {
            ...
            DamagePercentModAndSecondaryEffect_Item bigSpoon = new DamagePercentModAndSecondaryEffect_Item("ST_EntrenchingTool_ID", 50, true, false, true)
            {
                ...
                Description = "This party member now deals 50% more damage, but will move randomly Left or Right before using an ability.",
                IsShopItem = true,
                ShopPrice = 10,
                DoesPopUpInfo = false,
                StartsLocked = true,
            ...
            unlockCheck.AddUnlockData("Boyle", unlockData);

            ModdedAchievements unlockAchievement = new ModdedAchievements(bigSpoon.item._itemName, "Unlocked a new item.", ResourceLoader.LoadSprite("Ach_Deathmatch_Boyle", null, 32, null), achievementID);
            unlockAchievement.AddNewAchievementToCUSTOMCategory("AntagonistTitleLabel", "The Antagonist");

            LoadedAssetsHandler.GetCharacter("Boyle_CH").m_BossAchData.Add(new("Deathmatch_BOSS", achievementID));

            if (SorasToybox.extradebug.Value)
            {
                Debug.Log("Added the Entrenching Tool.");
            }
        }
    }
}
